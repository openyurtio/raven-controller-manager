...
func (r *GatewayReconciler) reconcileCentralGateway(ctx context.Context, centralGw *ravenv1alpha1.Gateway, currentGw *ravenv1alpha1.Gateway, gwList *ravenv1alpha1.GatewayList) (ctrl.Result, error) {
	...
	// 1. try to become central
	centralGw.Status.Central = true

	var gateways []string
	for _, v := range gwList.Items {
		if v.Name != centralGw.Name {
			gateways = append(gateways, v.Name)
		}
	}

	// 2. compute how many healthy active endpoint in central gateway
	chunkNum := 0
	...
	// 3. divide forward vpn connections by healthy active endpoint number
...
func (r *GatewayReconciler) findCentralGateway(gatewayList *ravenv1alpha1.GatewayList) *ravenv1alpha1.Gateway {
	// 1. find existing central gateway
	...
	// 2. try to become central gateway
	for _, v := range gatewayList.Items {
		for _, ep := range v.Spec.Endpoints {
			if !ep.UnderNAT {
				return &v
...
func generateForwards(gateways []string) []ravenv1alpha1.Forward {
	forwards := make([]ravenv1alpha1.Forward, 0)
	if len(gateways) > 1 {
		for _, v := range combinations.Combinations(gateways, 2) {
