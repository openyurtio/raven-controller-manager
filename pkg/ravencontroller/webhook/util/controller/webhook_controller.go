...
import (
	...
	"github.com/openyurtio/raven-controller-manager/pkg/ravencontroller/webhook/util/configuration"
	"github.com/openyurtio/raven-controller-manager/pkg/ravencontroller/webhook/util/generator"
	"github.com/openyurtio/raven-controller-manager/pkg/ravencontroller/webhook/util/writer"
)
...
func New(cli client.Client, kubeCli clientset.Interface) (*Controller, error) {
	c := &Controller{
		kubeClient:    kubeCli,
		runtimeClient: cli,
	...
	admissionRegistrationInformer.MutatingWebhookConfigurations().Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			conf := obj.(*admissionv1.MutatingWebhookConfiguration)
		...
		UpdateFunc: func(old, cur interface{}) {
			conf := cur.(*admissionv1.MutatingWebhookConfiguration)
			if conf.Name == mutatingWebhookConfigurationName {
	...
	admissionRegistrationInformer.ValidatingWebhookConfigurations().Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			conf := obj.(*admissionv1.ValidatingWebhookConfiguration)
		...
		UpdateFunc: func(old, cur interface{}) {
			conf := cur.(*admissionv1.ValidatingWebhookConfiguration)
			if conf.Name == validatingWebhookConfigurationName {
...
func (c *Controller) sync() error {
	...
	if err := configuration.Ensure(c.runtimeClient, certs.CACert); err != nil {
